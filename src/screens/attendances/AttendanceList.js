...
class AttendanceList extends Component {
    ...
    async componentDidMount() {
        ...
        .then( resp => resp.json())
        .then( res => this.setState({attendances: res}))
        .catch( error => console.log(error))
    }

    attendanceClicked = attendance => {
        this.props.navigation.navigate('AttendanceDetail', { attendance: attendance, token: token });  
    }
    ...
    addNewClicked = () => {
        ...
        this.props.navigation.navigate('AttendanceForm', {  attendance: newattendance, view_type: true, token: token });
    }
        ...
        return (
            <View style={styles.container}>
                <View style={{ marginBottom:20, height: 100, alignItems:"center", justifyContent: "center"}}>
                    <Text style={{fontSize:20, fontWeight:'bold'}}> Attendance List </Text>
                </View>
                <FlatList 
                    data={this.state.attendances}
                    renderItem={({item, index}) => (
                        <Text onPress={() => this.attendanceClicked(item)} style={[
