...
class ContractList extends Component {
    ...
    async componentDidMount() {
        ...
        .then( resp => resp.json())
        .then( res => this.setState({contracts: res}))
        .catch( error => console.log(error))
    }

    contractClicked = contract => {
        this.props.navigation.navigate('ContractDetail', { contract: contract, token: token });  
    }
    ...
    addNewClicked = () => {
        ...
        this.props.navigation.navigate('ContractForm', {  contract: newcontract, view_type: true, token: token });
    }
        ...
        return (
            <View style={styles.container}>
                <View style={{ marginBottom:20, height: 100, alignItems:"center", justifyContent: "center"}}>
                    <Text style={{fontSize:20, fontWeight:'bold'}}> Contract List </Text>
                </View>
                <FlatList 
                    data={this.state.contracts}
                    renderItem={({item, index}) => (
                        <Text onPress={() => this.contractClicked(item)} style={[
